// UI文言テーブル(ja/en)。app.js より前に読み込む。手で編集してよい(生成物ではない)。
// fetch не使わず JS 埋め込み = file:// でも動く(README/SPEC の制約)。
// 値は文字列(={k} 補間) か 関数(複数形など)。英語は move/moves の単複を関数で処理する。
window.NIKENZUME_STRINGS = {
  ja: {
    title: 'Tsugai',
    tagline: 'ふたつの部屋、ひとつの操作。',
    chapter: ({ n }) => `第${n}章`,
    modeNormal: '通常', modeAdvanced: '上級', modeThree: '3面', modeAdvancedLocked: 'Lv20 CLEARで解放', modeThreeLocked: '拡張パックで解放',
    // ボタン
    back: '戻る', reset: '最初から', answer: '答え', settings: '設定',
    navHome: 'ホーム', navMenu: 'ホームメニュー', navDaily: 'デイリー', navLevels: '一覧', navGoals: '目標', navStore: 'ストア',
    homePlay: 'プレイ', level: 'レベル',
    prevPuzzle: '前の問題', nextPuzzleNav: '次の問題', dailyBackToHub: 'デイリーへ',
    light: '光ヒント', nextMove: '次の手ヒント',
    hintLight: '光ヒント', hintNext: '次の手', hintAnswer: '答え',
    loginTitle: 'ログインボーナス', loginClaim: '受け取る',
    // 設定
    language: '言語', se: '効果音', haptics: '触覚フィードバック',
    setSecBasic: '基本', setSecSupport: 'サポート', setSecData: 'データ',
    resetProgress: '進捗をリセット', howto: '遊び方', about: 'このアプリについて',
    privacy: 'プライバシーポリシー', contact: 'お問い合わせ', review: 'レビューする',
    restore: '購入を復元',
    soon: '準備中',
    resetConfirm: '進捗(クリア・最短の記録)をすべて消去します。よろしいですか?',
    play: '再生', pause: '一時停止', prev: '前へ', next: '次へ', close: 'とじる',
    restart: '初手から', nextPuzzle: '次の問題へ', levels: '一覧へ', retryAgain: 'もう一度',
    // メッセージ
    controlsHint: '矢印キー / スワイプで両方の部屋が動く',
    missNote: 'ゴールは、ふたつ同時でなければ開かない',
    tapHint: '画面をタップ',
    noSolution: 'この局面からは進めません',
    // 動的(数字埋め込み)
    puzzlePar: ({ n }) => `${n}手詰`,
    levelMoves: ({ n }) => `${n}手`,
    levelLocked: '未解放',
    unlockByClear: ({ n }) => `レベル${n} CLEARで開放`,
    unlockAdvancedByClear: '通常レベル20をCLEARで解放',
    moveCount: ({ n }) => `${n}手`,
    clearedMoves: ({ n }) => `${n}手でクリア`,
    fewest: '最短手数です',
    fewestIs: ({ n }) => `最短は${n}手`,
    nextMoveHint: ({ n }) => `次の一手 ・ あと ${n} 手`,
    // 遊び方
    ht1title: 'すべての部屋を同時に動かす',
    ht1body: '通常・上級は2つ、3面は3つの部屋で遊びます。各部屋に球が1つずつあり、上下左右の入力はすべての球に同時に届きます。',
    ht2title: 'すべての球を同時にゴールへ',
    ht2body: '各部屋にある光の輪がゴールです。すべての球が同じ手で同時にゴールに入るとクリア。1つでも先にゴールに入ると失敗です。',
    ht3title: '壁がズレを生む',
    ht3body: '球が壁や外周にぶつかると、その球だけ止まり、ほかの部屋の球は動きます。このズレがパズルの核です。わざと壁にぶつけて、すべての球の位置関係を調整しましょう。',
    ht4title: '画面の操作',
    ht4body: '困ったときは、3つの操作を使えます。',
    ht4tip1: 'ぶつけるべき壁を光らせる',
    ht4tip2: '今の局面から最善の一手を示す',
    ht4tip3: '正解の手順を自動で再生する',
    ht4tip4: '初期位置に戻す',
    // プライバシーポリシー
    ppDate: '最終更新: 2026年6月',
    ppDataH: '収集するデータ', ppDataP: 'Tsugai はお名前・メールアドレス等の個人情報を収集しません。ゲームの進捗データはすべて端末内に保存されます。',
    ppLocalH: '端末内のデータ', ppLocalP: '以下のデータを端末内（localStorage）に保存します。',
    ppLocal1: 'クリア状況・最短手数の記録', ppLocal2: '表示設定',
    ppLocal3: '効果音・触覚のON/OFF設定', ppLocal4: '言語設定',
    ppLocalNote: 'これらはサーバーには送信されません。設定画面の「進捗をリセット」で全消去できます。',
    ppAdH: '広告', ppAdP: '本アプリでは Google AdMob を利用して広告を表示します。AdMob は広告配信のために広告ID・デバイス情報を収集することがあります。詳細は Google のプライバシーポリシーをご確認ください。',
    ppAdLink: 'Google プライバシーポリシー ↗',
    ppAnalyticsH: 'アナリティクス', ppAnalyticsP: 'アプリの改善のため、Firebase Analytics を利用して利用状況を匿名で収集します。個人を特定する情報は含まれません。',
    ppContactH: 'お問い合わせ', ppContactP: 'ご質問がある場合は、設定画面の「お問い合わせ」からご連絡ください。',
    // お問い合わせ
    contactDesc: 'バグの報告やご意見・ご要望はメールでお送りください。', contactSend: 'メールを送る',
    ht1cap: '通常・上級は2面、3面モードは3面。スワイプひとつですべての球が動く',
    ht2ok: 'クリア', ht2ng: '失敗',
    ht3before: '操作前', ht3after: '→ を入力',
    ht3same: '同じ位置', ht3gap: '壁で止まる = ズレ発生',
    ht4cap: '上部に「答え」、盤面の下に「最初から」と「光ヒント」',
    ht5title: '最短を目指す',
    ht5body: 'まずはCLEARを目指しましょう。クリアすると銀のCLEARエンブレム、最短手数でクリアすると金のBEST CLEARエンブレムが付きます。',
    // デイリーチャレンジ
    dailyTitle: 'デイリー',
    dailyNew: '挑戦する',
    dailyCleared: 'クリア済み',
    dailyBest: 'ベスト達成',
    dailyStreak: ({ n }) => `🔥 ${n}日連続`,
    dailyReward: 'クリア報酬',
    dailyStreakBonus: ({ n }) => `${n}日連続ボーナス`,
    dailyShare: 'シェア',
    dailyShareText: ({ day, moves, best, streak }) =>
      `Tsugai デイリー #${day}\n${moves}手でクリア${best ? ' ⭐' : ''}${streak > 0 ? `\n🔥 ${streak}日連続` : ''}`,
    copied: 'コピーしました',
    // デイリーハブ画面
    dailyDay: ({ n }) => `DAY ${n}`,
    dailyLevelBand: ({ n }) => `レベル ${n} 相当`,
    dailyRecordTime: 'タイム',
    dailyRefBest: '参考ベスト',
    dailyRefTag: '参考',
    dailyNoRecord: '—',
    // 統計・実績
    statCleared: 'クリア', statBest: 'ベスト',
    statDaily: 'デイリー',
    statDaysPlayed: 'クリア日数', statCurrentStreak: '現在の連続', statMaxStreak: '最長連続',
    statAchievements: '実績',
    ach_clear1: '初クリア', ach_clear10: '10問クリア', ach_clear50: '50問クリア', ach_clear100: '100問クリア',
    ach_best1: '初ベスト', ach_best10: 'ベスト10達成', ach_best50: 'ベスト50達成',
    ach_daily1: 'デイリー初クリア', ach_streak7: '7日連続', ach_streak14: '14日連続', ach_streak30: '30日連続',
    goalClear: 'クリア', goalLogin: 'ログイン', goalAchievements: '実績', goalContinuity: '継続', goalLoginStreak: '連続ログイン', goalDailyStreak: '連続デイリー',
    goalAchievementClear: ({ n }) => `レベル${n}をCLEAR`, goalAchievementBest: ({ n }) => `レベル${n}をBEST CLEAR`,
    goalAchievementPrefix: ({ n }) => `レベル${n}を`,
    goalLoginStreakAchievement: ({ n }) => `${n}日連続ログイン`, goalDailyStreakAchievement: ({ n }) => `${n}日連続デイリー`, goalDailyClearAchievement: ({ n }) => `デイリー${n}問クリア`,
    goalHintUseAchievement: ({ n }) => `ヒント${n}回使用`, goalAnswerUseAchievement: ({ n }) => `答え${n}回使用`,
    // ストア
    store: 'ストア',
    storeThreePack: '3面拡張パック', storeThreePackDesc: '広告除去と3面ステージを解放します',
    storeAdFree: '広告除去', storeAdFreeDesc: '広告なしで、ヒントと答えをすぐに使えます',
    storeSkinPack: 'スキンパック', storeSkinPackDesc: '新しい配色テーマを準備中です',
    storeComingSoon: 'Coming soon',
    storeOwned: '購入済み', storeIncluded: 'パックに含まれます',
    storeThanks: '購入ありがとうございます!',
    storeRestored: '購入を復元しました', storeNothingToRestore: '復元できる購入はありません',
    storeError: '購入を完了できませんでした',
  },
  en: {
    title: 'Tsugai',
    tagline: 'Two rooms, one move.',
    chapter: ({ n }) => `Chapter ${n}`,
    modeNormal: 'Normal', modeAdvanced: 'Advanced', modeThree: 'Three', modeAdvancedLocked: 'Clear Lv.20', modeThreeLocked: 'Unlock with pack',
    // ボタン
    back: 'Back', reset: 'Restart', answer: 'Answer', settings: 'Settings',
    navHome: 'Home', navMenu: 'Home menu', navDaily: 'Daily', navLevels: 'Levels', navGoals: 'Goals', navStore: 'Store',
    homePlay: 'Play', level: 'Level',
    prevPuzzle: 'Previous level', nextPuzzleNav: 'Next level', dailyBackToHub: 'Daily',
    light: 'Light hint', nextMove: 'Next move',
    hintLight: 'Light', hintNext: 'Next move', hintAnswer: 'Answer',
    loginTitle: 'Login Bonus', loginClaim: 'Claim',
    // settings
    language: 'Language', se: 'Sound', haptics: 'Haptics',
    setSecBasic: 'General', setSecSupport: 'Support', setSecData: 'Data',
    resetProgress: 'Reset progress', howto: 'How to play', about: 'About',
    privacy: 'Privacy policy', contact: 'Contact', review: 'Rate',
    restore: 'Restore purchases',
    soon: 'Coming soon',
    resetConfirm: 'This will erase all progress (clear & best records). Continue?',
    play: 'Play', pause: 'Pause', prev: 'Prev', next: 'Next', close: 'Close',
    restart: 'Retry', nextPuzzle: 'Next puzzle', levels: 'Levels', retryAgain: 'Try again',
    controlsHint: 'Arrow keys or swipe move both rooms',
    missNote: 'The goal opens only when both arrive together.',
    tapHint: 'Tap to continue',
    noSolution: 'No solution from here',
    puzzlePar: ({ n }) => `Solve in ${n}`,
    levelMoves: ({ n }) => `${n}`,
    levelLocked: 'Locked',
    unlockByClear: ({ n }) => `Clear level ${n} to unlock`,
    unlockAdvancedByClear: 'Clear Normal Level 20 to unlock',
    moveCount: ({ n }) => `${n} ${n === 1 ? 'move' : 'moves'}`,
    clearedMoves: ({ n }) => `Cleared in ${n} ${n === 1 ? 'move' : 'moves'}`,
    fewest: 'Fewest moves!',
    fewestIs: ({ n }) => `Fewest is ${n} ${n === 1 ? 'move' : 'moves'}`,
    nextMoveHint: ({ n }) => `Next · ${n} to go`,
    // How to play
    ht1title: 'Move every room together',
    ht1body: 'Normal and Advanced use two rooms; Three uses three. Each room has one ball, and every move—up, down, left, or right—reaches every ball at once.',
    ht2title: 'Reach every goal together',
    ht2body: 'The glowing ring in each room is the goal. Clear the puzzle by landing every ball on its goal in a single move. If even one ball arrives early, that\'s a miss.',
    ht3title: 'Walls create the gap',
    ht3body: 'When a ball hits a wall or the edge, it stays put while balls in the other rooms keep moving. This gap is the heart of every puzzle. Deliberately bumping into walls lets you adjust the positions of every ball.',
    ht4title: 'Screen controls',
    ht4body: 'When you get stuck, use one of these three controls.',
    ht4tip1: 'Highlights the walls to bump',
    ht4tip2: 'Shows the best next move',
    ht4tip3: 'Auto-plays the solution',
    ht4tip4: 'Return to the starting position',
    // Privacy policy
    ppDate: 'Last updated: June 2026',
    ppDataH: 'Data we collect', ppDataP: 'Tsugai does not collect personal information such as your name or email address. Game progress is stored entirely on your device.',
    ppLocalH: 'On-device data', ppLocalP: 'The following data is saved locally (localStorage):',
    ppLocal1: 'Clear status & best move counts', ppLocal2: 'Display preference',
    ppLocal3: 'Sound & haptics on/off', ppLocal4: 'Language setting',
    ppLocalNote: 'None of this is sent to a server. Use "Reset progress" in Settings to erase it all.',
    ppAdH: 'Advertising', ppAdP: 'This app displays ads via Google AdMob. AdMob may collect your advertising ID and device information to serve ads. See Google\'s Privacy Policy for details.',
    ppAdLink: 'Google Privacy Policy ↗',
    ppAnalyticsH: 'Analytics', ppAnalyticsP: 'To improve the app, we collect anonymous usage data via Firebase Analytics. No personally identifiable information is included.',
    ppContactH: 'Contact', ppContactP: 'If you have questions, please reach out via "Contact" in the Settings screen.',
    // Contact
    contactDesc: 'To report bugs or share feedback, please send us an email.', contactSend: 'Send email',
    ht1cap: 'Normal and Advanced have two boards; Three has three. One swipe moves every ball.',
    ht2ok: 'Clear', ht2ng: 'Miss',
    ht3before: 'Before', ht3after: 'Swipe →',
    ht3same: 'Same position', ht3gap: 'Blocked by wall = gap created',
    ht4cap: 'Answer is at the top; Restart and Light hint are below the boards.',
    ht5title: 'Aim for the fewest moves',
    ht5body: 'First, aim for a clear. A clear earns a silver CLEAR emblem; clearing in the fewest moves earns a gold BEST CLEAR emblem.',
    // Daily challenge
    dailyTitle: 'Daily',
    dailyNew: 'Play',
    dailyCleared: 'Cleared',
    dailyBest: 'Best!',
    dailyStreak: ({ n }) => `🔥 ${n} day streak`,
    dailyReward: 'Reward',
    dailyStreakBonus: ({ n }) => `${n}-day streak bonus`,
    dailyShare: 'Share',
    dailyShareText: ({ day, moves, best, streak }) =>
      `Tsugai Daily #${day}\nCleared in ${moves} ${moves === 1 ? 'move' : 'moves'}${best ? ' ⭐' : ''}${streak > 0 ? `\n🔥 ${streak} day streak` : ''}`,
    copied: 'Copied',
    // Daily hub screen
    dailyDay: ({ n }) => `DAY ${n}`,
    dailyLevelBand: ({ n }) => `~ Level ${n}`,
    dailyRecordTime: 'Time',
    dailyRefBest: 'Ref. best',
    dailyRefTag: 'ref',
    dailyNoRecord: '—',
    // Stats & achievements
    statCleared: 'Cleared', statBest: 'Best',
    statDaily: 'Daily',
    statDaysPlayed: 'Days cleared', statCurrentStreak: 'Current streak', statMaxStreak: 'Best streak',
    statAchievements: 'Achievements',
    ach_clear1: 'First clear', ach_clear10: '10 puzzles cleared', ach_clear50: '50 puzzles cleared', ach_clear100: '100 puzzles cleared',
    ach_best1: 'First best', ach_best10: '10 bests', ach_best50: '50 bests',
    ach_daily1: 'First daily clear', ach_streak7: '7-day streak', ach_streak14: '14-day streak', ach_streak30: '30-day streak',
    goalClear: 'Clear', goalLogin: 'Login', goalAchievements: 'Achievements', goalContinuity: 'Streaks', goalLoginStreak: 'Login streak', goalDailyStreak: 'Daily streak',
    goalAchievementClear: ({ n }) => `Clear Level ${n}`, goalAchievementBest: ({ n }) => `Best-clear Level ${n}`,
    goalAchievementPrefix: ({ n }) => `Level ${n} `,
    goalLoginStreakAchievement: ({ n }) => `${n}-day login streak`, goalDailyStreakAchievement: ({ n }) => `${n}-day daily streak`, goalDailyClearAchievement: ({ n }) => `Clear ${n} daily ${n === 1 ? 'puzzle' : 'puzzles'}`,
    goalHintUseAchievement: ({ n }) => `Use hints ${n} ${n === 1 ? 'time' : 'times'}`, goalAnswerUseAchievement: ({ n }) => `Use answers ${n} ${n === 1 ? 'time' : 'times'}`,
    // Store
    store: 'Store',
    storeThreePack: 'Three-Board Pack', storeThreePackDesc: 'Removes ads and unlocks the three-board stages',
    storeAdFree: 'Remove Ads', storeAdFreeDesc: 'Play ad-free — use hints and answers instantly',
    storeSkinPack: 'Skin Pack', storeSkinPackDesc: 'New color themes are in the works',
    storeComingSoon: 'Coming soon',
    storeOwned: 'Purchased', storeIncluded: 'Included in the pack',
    storeThanks: 'Thank you for your purchase!',
    storeRestored: 'Purchases restored', storeNothingToRestore: 'No purchases to restore',
    storeError: 'Purchase could not be completed',
  },
};
